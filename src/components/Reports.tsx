...
import { Database, calculateAge, getCategory } from '../utils/database';
import {
  format,
  startOfWeek,
  endOfWeek,
...
export const Reports: React.FC = () => {
  ...
  // Set the time to the very end of the selected day for accurate filtering
  ...
  // Recent attendance with student details
  const recentAttendance = filteredRecords
    .sort(
      (a, b) =>
        new Date(b.checkinTimestamp).getTime() -
        new Date(a.checkinTimestamp).getTime()
    )
    .slice(0, 20)
    .map((record) => {
      const student = Database.getStudentById(record.studentId);
  ...
  const stats = [
    ...
    },
    {
        ...
        value: Math.round(totalAttendance / numberOfActiveSessions) || 0,
        icon: TrendingUp,
        color: 'bg-purple-500',
      },
    {
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="font-bold text-gray-900 mt-1 text-[clamp(1.5rem,5vw,2.25rem)]">
                  {stat.value}
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {[
              { time: '09:30', label: '9:30 AM Service', count: sessionBreakdown['09:30'] },
              { time: '11:00', label: '11:00 AM Service', count: sessionBreakdown['11:00'] },
              { time: '14:00', label: '2:00 PM Service', count: sessionBreakdown['14:00'] },
              { time: '16:00', label: '4:00 PM Service', count: sessionBreakdown['16:00'] },
            ].map((session) => {
              const percentage =
                totalAttendance > 0 ? (session.count / totalAttendance) * 100 : 0;
              return (
                <div key={session.time} className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-sm font-medium text-gray-700">
                      {session.label}
                    </span>
                    <span className="text-sm text-gray-600">
                      {session.count} ({Math.round(percentage)}%)
                    </span>
                  </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {Object.keys(categoryBreakdown).length > 0 ? Object.entries(categoryBreakdown).map(([category, count]) => {
              const percentage =
                totalAttendance > 0 ? (count / totalAttendance) * 100 : 0;
              return (
              ...
              {recentAttendance.map((record) => (
                <tr key={record.id} className="border-b border-gray-100">
                  ...
                  <td className="py-3 text-sm text-gray-600">
                    {record.student
                      ? getCategory(calculateAge(record.student.dateOfBirth))
                      : 'N/A'}
                  </td>
                  <td className="py-3 text-sm text-gray-600">
                    {record.sessionTime === '09:30'
                      ? '9:30 AM'
                      : record.sessionTime === '11:00'
                      ? '11:00 AM'
                      : record.sessionTime === '14:00'
                      ? '2:00 PM'
                      : '4:00 PM'}
                  </td>
                  <td className="py-3 text-sm text-gray-600">
